...
def app():
    ...
    for i in range(1, 20):
        ...
        sfs_clone = clone(sfs) # make a clone of the selector to avoid fitting again
        sfs_clone.n_features_to_select = i # set the number of features to select
        sfs_clone.fit(X, y) # fit the sequential forward feature selector
        ...
        # create a new instance of the logistic regression model
        lr = LogisticRegression()
        sbs_clone = clone(sbs) # make a clone of the selector to avoid fitting again
        sbs_clone.n_features_to_select = i # set the number of features to select
        sbs_clone.fit(X, y) # fit the sequential backward feature selector
        # get the selected features and their corresponding scores
